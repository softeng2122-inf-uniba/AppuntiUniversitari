...
int main()
{
    ...
    if(pvgg==NULL)
        printf("\n Il file Viaggi.txt non può essere aperto\n");
    else
        caricafile();
    fclose(pvgg);
...
void chilometrieffettuati()
{
    ...
    klm=klm/1000;
    printf("\n I kilometri effettuati verso la località sono %.2f",klm);
}
void data()
{
   char max[21]="fd";
   ...
   printf("\n la data dell'ultimo viaggio verso la localtà selezionata è %s",max);
}
void costototale()
{
    int i;
    ...
    printf("\n il costo totale dei viaggi è %f",costotot);
}
