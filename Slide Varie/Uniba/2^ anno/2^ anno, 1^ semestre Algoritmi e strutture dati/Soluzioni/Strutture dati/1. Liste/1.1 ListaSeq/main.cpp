...
posizione recuperaPos(tipoelem a, ListaSeq l){ //verrà richiamata sempre quando l NON è vuota
   posizione p = l.primoLista();
   tipoelem b;
   bool trovato = false;
   while (!(l.fineLista(p) || trovato)){
...
void inserisciListaOrd(ListaSeq &l){
   int i = 0;
   char risp = 's';
   tipoelem a;
   posizione p;
...
bool fondiLista(ListaSeq l, ListaSeq m, ListaSeq &n){ //le liste devono essere ordinate
   posizione p = l.primoLista();
