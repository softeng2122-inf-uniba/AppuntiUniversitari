...
void visitaOutputB(Alberobin &tree, Alberobin::nodo n, int counter, int &numnodi)
...
//e stampa del contenuto
	{
      ...
      cout<<tree.legginodo(n)<<"\n";
   	...
   	if (!tree.sinistrovuoto(n))
        	visitaOutputB(tree, tree.figliosinistro(n),counter+1,numnodi);
   	else
      	{
		      for(i=0;i<counter;i++)
					cout<<"    ";
      ...
      if (!tree.destrovuoto(n))
     		visitaOutputB(tree, tree.figliodestro(n),counter+1,numnodi);
   	else
      	{
		      for(i=0;i<counter;i++)
					cout<<"    ";
